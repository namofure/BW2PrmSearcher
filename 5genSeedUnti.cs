...
namespace Prm
{
    public struct InSeedData
    {
        ...
        public uint EnTimer0;
    }
    public static class PID
    {
        public static List<uint> Values = new List<uint>();
        public static DateTime InDt;
        public static DateTime EnDt;
        public static DateTime Dt;
        public static TimeSpan increment;
        public static uint InTimer0;
        public static uint EnTimer0;
        public static uint Timer0;
        ...
        static PID()
        {
            ...
            InTimer0 = uint.Parse(lines[4].Split("0x")[1].Trim(), NumberStyles.HexNumber);
            EnTimer0 = uint.Parse(lines[5].Split("0x")[1].Trim(), NumberStyles.HexNumber);
            InDt = DateTime.Parse(lines[15].Split('=')[1].Trim());
            EnDt = DateTime.Parse(lines[16].Split('=')[1].Trim());
            int AddHours = int.Parse(lines[17].Split('=')[1].Trim());
            int AddMinutes = int.Parse(lines[18].Split('=')[1].Trim());
            int AddSeconds = int.Parse(lines[19].Split('=')[1].Trim());
            PIDCount = int.Parse(lines[20].Split('=')[1].Trim());
            CountFlag = int.Parse(lines[21].Split('=')[1].Trim());
        ...
        public static InSeedData GenSeed()
        {
            ...
            Data[6] = (Values[7]);
            Data[7] = toLittleEndian(((Values[8] ^ Values[9])) ^ toLittleEndian(Values[6]));
            Data[8] = (Date);
            Data[9] = (Time);
            Data[10] = 0;
            Data[11] = 0;
            Data[12] = toLittleEndian(Values[10]);
            Data[13] = (Values[11]);
            Data[14] = (Values[12]);
            Data[15] = (Values[13]);
